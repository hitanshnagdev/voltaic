...
export const parseSpecChecklist = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // NOTE: previously fanned out to assigned submittals here, kicking
    // off guided extraction the moment the checklist landed. Removed
    // when compliance runs were gated behind an explicit /compare CTA
    // — assignment + checklist parse are no-cost setup; the paid
    // extraction step is now user-initiated.
